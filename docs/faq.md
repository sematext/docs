...
You can reach us at <support@sematext.com> or tweet
[@sematext](http://twitter.com/sematext).
...
See [integrations](../integrations). You can also send [custom metrics](custom-metrics).

**Does Sematext integrate with ChatOps like Slack and HipChat? How about PagerDuty or custom WebHooks?**

Yes, see [integrations](/integrations) and [alerts](/alerts).
...
number in `/opt/spm/spm-monitor/bin/spm-monitor-starter.shd`, with the
...
in chat or via email (support@sematext.com) if you'd like to get more
info and help around making this adjustment.
...
2.  Change the value of `SPM\_MONITOR\_JMX\_PARAMS` in
...
to various apps), you'd give them the BILLING\_ADMIN role.
...
Yes. Simply add or edit the SPM\_MONITOR\_LOGGING\_LEVEL property in
...
Yes, we offer a pre-payment option through invoicing.  When you
