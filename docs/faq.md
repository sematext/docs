...
**A**: There are no limits on the number of servers, nodes, or
...
**A**: See [integrations](../integrations). You can also send [custom metrics](custom-metrics).

**Does Sematext integrate with ChatOps like Slack and HipChat? How about PagerDuty or custom WebHooks?**

**A:** Yes, see [integrations](/integrations) and [alerts](/alerts).
...
**Can I use SPM to monitor my service which runs on Windows or Mac?**
...
**A:** Yes. The following steps are needed:
...
one parameter, token of SPM application you want to uninstall):
...
**A**: Yes, just provide the proxy info in the
...
**A**: If you are having issues with Sematext Monitoring, you can
...
**A**: Sematext Monitoring reports both free and reserved disk space
...
**A:** Yes, although we recommend using HTTPS. Since version 1.22.0,
SPM client by default uses HTTPS to send metrics data to Sematext. If
you prefer to use HTTP instead (for example, if you are running SPM on
premises or if you don't need metric data to be encrypted when being
sent to SPM over the Internet), you can adjust that in
