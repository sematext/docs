...
3. After installing the agent, discovered logs or metrics from Elasticsearch services detected on the host will start shipping automatically.
...
5. If you've created an Elasticsearch Monitoring App and want to collect Elasticsearch logs as well, or vice versa, click on the **Create Logs App** button from the left menu panel. This will take you to the **Create Logs App** (or Monitoring App) page. When you select **Use Existing Agent**, all discovered log sources from Elasticsearch services are listed, allowing you to set up log and metric shipping easily without any additional installation.
