...
The Sematext Infra Integration  uses [Sematext Agent](https://sematext.com/docs/agents/sematext-agent/) to collect metrics, events, processes, packages for bare-metal servers, virtual machines and containers. Container monitoring is supported for Docker, containerd, k8s, swarm, nomad, ecs, eks, aks, gke and more.
...
First create an [Infra Monitoring App in Sematext Cloud](https://apps.sematext.com/ui/monitoring-create).
...
It comes prepared for two kinds of environments:
...
In both cases, installing the agent will start gathering infrastructure data. Additionally, it will enable [Monitoring](../monitoring/autodiscovery) and [Logs Autodiscovery](../logs/discovery/intro/) features which make it possible to automatically start monitoring any of supported [Sematext integrations](./index) or to ship logs produced by any process or container.
