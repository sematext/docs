...
5. After installing the agent and configuring RabbitMQ, discovered logs or metrics from RabbitMQ services detected on the host will start shipping automatically.
6. If you've created an RabbitMQ Monitoring App and want to collect RabbitMQ logs as well, or vice versa, click on the **Create Logs App** button from the left menu panel. This will take you to the **Create Logs App** (or Monitoring App) page. When you select **Use Existing Agent**, all discovered log sources from RabbitMQ services are listed, allowing you to set up log and metric shipping easily without any additional installation.
