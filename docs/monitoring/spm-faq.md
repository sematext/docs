...
For sematext-agent-java, you can find those files with:
...
For st-agent, you can configure following properties in `/opt/spm/properties/sta.yml`
...
frequency to 30 seconds, for example, for sematext-agent-java simply add the following line to
...
For st-agent, set `interval` property `/opt/spm/properties/sta.yml`.
...
For sematext-agent-java, simply add or edit the `SPM_MONITOR_LOGGING_LEVEL` property in
...
For st-agent, edit `logging.level` property in `/opt/spm/properties/sta.yml` file.
```bash
