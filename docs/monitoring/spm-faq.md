...
(otherwise it logs detailed info about its monitoring lifecycle). To do that, change
properties files of your Monitoring Apps. 

For Java agent, you can find those files with:
...
For Infra agent, you can configure following properties in `/opt/spm/properties/sta.yml`
...
frequency to 30 seconds, for example, for Java agent simply add the following line to
...
For Infra agent, set `interval` property `/opt/spm/properties/sta.yml`.
To set it to 30 seconds, set the value of interval to `30s`.
...
**How much memory is standalone Java agent using and can it be adjusted?**
...
Yes. SPM client provides two scripts that can be used to move SPM client 
...
For Java agent, simply add or edit the `SPM_MONITOR_LOGGING_LEVEL` property in
...
For Infra agent, edit `logging.level` property in `/opt/spm/properties/sta.yml` file.
...
SPM client by default uses HTTPS to send metrics data to Sematext. If you
